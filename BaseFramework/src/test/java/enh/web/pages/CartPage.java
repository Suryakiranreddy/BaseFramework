	...
	public static void addItemInCart(GiftBy giftBy) throws Exception{
		if(giftBy==GiftBy.FLOWER_AND_CAKE){
			...
			/*	waitForClickable(btnAddToCart);
				click(By.xpath("//*[@id='attr-1']/div[1]/h3"));
				
				doubleClick(txtPinCode);
				executeStep(inputText(txtPinCode, Constants.PINCODE),"Input pincode");
				click(btnCheckPin);
				pause(3000);
				click(By.xpath("//*[@id='domestic-del']/div/div/div/div[1]/div[2]/div/label"));
				executeStep(click(btnAddToCart), 
						"Click: "+"ADD TO CART");*/
				
				//inputPinCode("500060");
				//getDriver().findElement(txtPinCode).sendKeys(Keys.TAB);
				inputText(txtPinCode, "500060");
				Robot r = new Robot();
				r.keyPress(KeyEvent.VK_ENTER);
				r.keyRelease(KeyEvent.VK_ENTER);
				Thread.sleep(5000);
				clickJS(By.xpath("//span[contains(.,'Next Day Delivery')]"));
				
	}
	
	public static void inputPinCode(String pin) throws Exception{
		doubleClick(txtPinCode);
		executeStep(inputTextJS(txtPinCode, pin),"Input pincode:"+pin);
